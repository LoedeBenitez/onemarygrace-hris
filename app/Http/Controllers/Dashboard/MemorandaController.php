...
class MemorandaController extends Controller
{
    ...
    public static function getRules()
    {
        return [
            ...
            'to' => 'required|string',
            'effective_date' => 'date|date_format:Y-m-d',
            'file' => 'nullable',
    ...
    public function onCreate(Request $request)
    {
        ...
        try {
            ...
            //     }
            // }
            $this->memoranda = new Memoranda();
            $this->memoranda->fill($fields);
            $this->memoranda->save();
            return ResponseHelper::dataResponse('success', 201, EnglishLanguage::onCreate('Memoranda', 1), null);
        } catch (\Exception $exception) {
            return ResponseHelper::dataResponse('error', 400, EnglishLanguage::onCreate('Memoranda', 0), $exception->getMessage());
        }
    }
    ...
    public function onUpdateById(Request $request, $id)
    {
        ...
        try {
            ...
            //     }
            // }
            $this->memoranda = Memoranda::findOrFail($id);
            if ($this->memoranda) {
                $fields['updated_by_id'] = $fields['created_by_id'];
                $this->memoranda->update($fields);
                return ResponseHelper::dataResponse('success', 200, EnglishLanguage::onUpdate('Memoranda', 1), $this->memoranda);
            }
            return ResponseHelper::dataResponse('error', 404, EnglishLanguage::onExist('Memoranda', 3), null);
        } catch (\Exception $exception) {
            return ResponseHelper::dataResponse('error', 400, EnglishLanguage::onCreate('Memoranda', 0), $exception->getMessage());
        }
    }
    ...
    public function onGetAll(Request $request)
    {
        ...
        CheckAuth::auth($bearerToken);
        try {
            $memorandaData = Memoranda::all();
            if ($memorandaData->count() !== 0) {
                $reconstructedList = [];
                foreach ($memorandaData as $data) {
                    $reconstructedList[] = $this->onReconstructedList($data->id, $bearerToken);
                }
                return ResponseHelper::dataResponse('success', 200, EnglishLanguage::onGet('Memoranda', 1), $reconstructedList);
            }

        } catch (\Exception $exception) {
            return ResponseHelper::dataResponse('error', 400, EnglishLanguage::onGet('Memoranda', 0), $exception->getMessage());
        }
    }
    ...
    public function onGetById(Request $request, $id)
    {
        ...
        CheckAuth::auth($bearerToken);
        try {
            $memorandaData = Memoranda::find($id);

            if ($memorandaData) {
                $memorandaData = $this->onReconstructedList($memorandaData->id, $bearerToken);
                return ResponseHelper::dataResponse('success', 200, EnglishLanguage::onGet('Memoranda', 1), $memorandaData);
            }
            return ResponseHelper::dataResponse('success', 200, EnglishLanguage::onExist('Memoranda', 3), null);
        } catch (\Exception $exception) {
            return ResponseHelper::dataResponse('error', 400, EnglishLanguage::onGet('Memoranda', 0), $exception->getMessage());
        }
    }
    ...
    public function onGetPaginatedList(Request $request)
    {
        ...
        $typeMessage = $type == 1 ? 'Memoranda' : 'Feeds';
        try {
            $query = Memoranda::where('type', $type)->orderBy('created_at', 'desc');
    ...
    public function onDeleteById(Request $request, $id)
    {
        ...
        CheckAuth::auth($bearerToken);
        try {
            $deletedRows = Memoranda::destroy($id);
            if ($deletedRows) {
                return ResponseHelper::dataResponse('success', 200, EnglishLanguage::onDelete('Memoranda', 1), null);
            }
            return ResponseHelper::dataResponse('error', 404, EnglishLanguage::onExist('Memoranda', 3), null);
        } catch (\Exception $exception) {
            return ResponseHelper::dataResponse('error', 500, EnglishLanguage::onDelete('Memoranda', 0), $exception->getMessage());
        }
    }

    public function onReconstructedList($id, $token)
    {
        $data = Memoranda::findOrFail($id);
